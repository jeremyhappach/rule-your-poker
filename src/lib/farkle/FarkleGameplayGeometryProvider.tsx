/**
 * Wave 5D — FarkleGameplayGeometryProvider.
 *
 * Wraps the Farkle felt subtree, resolves anchored descriptors once
...
import {
  getFarkleArtifactDescriptors,
} from "@/lib/farkle/artifactDescriptors";
import { useDraftedGeometryOverrides, applyGeometryOverrides } from "@/lib/geometryLab/store";

export interface FarkleGameplayGeometryContextValue {
...
const FarkleGameplayGeometryContext =
  createContext<FarkleGameplayGeometryContextValue>({
...
export interface FarkleGameplayGeometryProviderProps {
  children: ReactNode;
}

export function FarkleGameplayGeometryProvider({
  children,
}: FarkleGameplayGeometryProviderProps) {
  ...
  const value = useMemo<FarkleGameplayGeometryContextValue>(() => {
    if (!geometry) {
      return {
        placementsById: EMPTY_MAP,
  ...
  return (
    <FarkleGameplayGeometryContext.Provider value={value}>
      {children}
    </FarkleGameplayGeometryContext.Provider>
  );
}

export function useFarkleGameplayGeometry(): FarkleGameplayGeometryContextValue {
  return useContext(FarkleGameplayGeometryContext);
}
