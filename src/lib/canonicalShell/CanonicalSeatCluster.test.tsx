...
describe('CanonicalSeatCluster — inward name projection (3C.4a follow-up)', () => {
  it('renders the name BELOW the chip for top-row slots (inward = down)', () => {
    renderInLayer(
      <CanonicalSeatCluster slot={2} position={2} name="Alice" chipValue="$1" />,
    );
    const below = container.querySelector('[data-canonical-seat-below]');
    expect(below?.querySelector('[data-canonical-seat-name-row]')).not.toBeNull();
    const above = container.querySelector('[data-canonical-seat-above]');
    expect(above?.querySelector('[data-canonical-seat-name-row]') ?? null).toBeNull();
    expect(container.querySelector('[data-canonical-seat-inner-name]')).toBeNull();
  });

  it('renders the name ABOVE the chip for bottom-row slots (inward = up)', () => {
    renderInLayer(
      <CanonicalSeatCluster slot={0} position={3} name="Alice" chipValue="$1" />,
      { seats: [{ position: 3, occupied: true }] },
    );
    const above = container.querySelector('[data-canonical-seat-above]')!;
    expect(above.querySelector('[data-canonical-seat-name-row]')).not.toBeNull();
    expect(container.querySelector('[data-canonical-seat-inner-name]')).toBeNull();
  });

  it('renders the name to the INNER (right) side for mid-left slot 1', () => {
    renderInLayer(
      <CanonicalSeatCluster slot={1} position={2} name="Alice" chipValue="$1" />,
    );
    const inner = container.querySelector('[data-canonical-seat-inner-name]')!;
    expect(inner).not.toBeNull();
    expect(inner.className).toContain('left-full');
    expect(inner.querySelector('[data-canonical-seat-name-row]')).not.toBeNull();
  });

  it('renders the name to the INNER (left) side for mid-right slot 4', () => {
    renderInLayer(
      <CanonicalSeatCluster slot={4} position={6} name="Alice" chipValue="$1" />,
      { seats: [{ position: 6, occupied: true }] },
    );
    const inner = container.querySelector('[data-canonical-seat-inner-name]')!;
    expect(inner.className).toContain('right-full');
  });

  it('keeps [data-chip-center] rect untouched regardless of nameSide', () => {
    // Just assert the chip-center marker is always present for any slot —
    // proves the projection variants do not gate or move the anchor.
    for (const slot of [0, 1, 2, 3, 4, 5] as const) {
      ...
      root = createRoot(container);
      renderInLayer(
        <CanonicalSeatCluster slot={slot} position={slot + 1} name="X" chipValue="$1" />,
        { seats: [{ position: slot + 1, occupied: true }] },
      );
      expect(container.querySelector(`[data-chip-center="${slot + 1}"]`)).not.toBeNull();
    }
  });
});
