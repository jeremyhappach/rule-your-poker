...
it('caps the batch, continues beyond the old expiry and distinguishes missing server timing from zero', async () => {
  const t = await import('./livePlayTiming');
  for (let i = 0; i < 500; i++) t.recordCardScan(identity, 20);
  ...
  vi.setSystemTime(new Date('2027-01-01')); expect(t.liveTimingEnabled()).toBe(true);
  mocks.send.mockClear(); t.recordCardScan(identity, 20); await vi.advanceTimersByTimeAsync(60_001); expect(mocks.send).toHaveBeenCalledOnce();
});
it('retains failed batches with a stable UUID and never uploads another viewer', async () => {
  ...
  t.recordCardScan(identity, 1); await vi.advanceTimersByTimeAsync(60_001);
