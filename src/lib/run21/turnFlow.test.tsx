...
describe('Run21 live sequential turn flow',()=>{
 it.each([false,true])('admits one public board with a frozen clock (bot first: %s)',reverse=>{
  ...
  expect(html).toContain('data-playing-card-face');expect(html.match(/disabled=""/g)).toHaveLength(5);
  const timer=renderToStaticMarkup(<Run21Timer view={view} now={100000}/>);
  expect(timer).toContain('Speed 250');expect(timer).toContain('data-run21-time-remaining="25000"');
 ...
 it('keeps admission, Pass, invalid actions, duplicate requests and reconnect unstarted; starts once on placement',()=>{
  ...
  expect(m.rounds[0].boards[human]).toMatchObject({startedAt:200000,deadline:225000});
  ...
  expect(speedAt(m.rounds[0].boards[human],m.config,200100)).toBe(249);
 ...
 it('orders score announcement, calculation, count-up, board clear and next admission without starting the next clock',()=>{
  ...
  expect(result).toMatchObject({aggregate:100,multiplier:200,speed:249,score:49800});
  ...
  expect(html).toContain('200 × 249 = 49,800');
  expect([200,2700,5200].map(at=>displayedScore(v,human,at))).toEqual([0,24900,49800]);
  expect(m.cumulative[human]).toBe(49800);
  ...
  m=act(m,bot,{type:'place',column:0},5300);m=act(m,bot,{type:'expire'},30300);
  ...
  m=advanceScorePresentation(m,30300+SCORE_PRESENTATION_MS);
  expect(m.rounds[0].revealed).toBe(true);expect(m.cumulative[human]).toBe(49800);
  expect(JSON.stringify(m.rounds[0].boards[human])).toBe(frozen);
