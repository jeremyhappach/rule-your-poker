...
export const CribbageMobileGameTable = ({
      ...
      load: async (source) => {
        ...
        try {
          ...
          recordGameFreezeTrace('cribbage.dealer_selection_fetch.finished', {
            ...
            hasState: !!state,
            isComplete: state?.isComplete ?? null,
          });
          return state;
        } catch (error) {
  ...
  useEffect(() => {
    ...
    const exactStateLoader = createLatestAuthoritativeLoader({
      ...
      apply: (newState, source) => {
        ...
        handleStateUpdate(newState, source);
      },
      onError: (error, source) => {
  ...
  useEffect(() => {
    ...
    authoritativeCutPresentation.isPeggingPresentationBlocked,
    cribbageState,
    isProcessing,
    players,
    roundId,
  ...
  useEffect(() => {
    if (!authoritativeCutPresentation.isHistoricalExposedCut) return;
    // Keep the local renderer converged with the authoritative recovery
    // decision. Rendering itself also consumes this same derivation, so the
    // spotlight and controls cannot remain blocked while this effect settles.
    setDiscardsSettledInHand((settled) =>
      Math.max(settled, authoritativeCutPresentation.settledCribCount),
    );
    if (
    ...
    authoritativeCutPresentation.isHistoricalExposedCut,
    authoritativeCutPresentation.settledCribCount,
    cutRevealCompletedHandKey,
    cutRevealHandKey,
  ]);
  ...
  */
  ...
  const resolvedDiscardsSettledInHand = renderedCutPresentation.settledCribCount;
  const cutRevealPresentationBlocked = renderedCutPresentation.isPeggingPresentationBlocked;
  ...
  return (
    <DealRuntimeMaybe
      ...
      /* Contract A (refresh/rejoin) with parent-owned entry-mode provenance:
         GAMEPLAY (suppress historical replay) only when this route mount
         entered into an already-existing canonical hand (entryMode ===
         'historical-entry') AND the authoritative phase is already past
         opening-deal. Any hand introduced *after* the parent route
         mounted — including hand 1 of a newly started game witnessed by
         an already-present client — is 'live-transition' and initializes
         PRE_DEAL so the normal opening transport runs. */
      initialPhase={
        entryMode === 'historical-entry' &&
        cribbageState?.phase &&
            ...
            <CribbageAnchoredCribCutMount
              ...
              reservedCribLayoutCount={(() => {
                ...
                const projected = Math.max(
                  authoritative,
                  intentProjected,
                  resolvedDiscardsSettledInHand + inFlight,
                );
              ...
              deferCutReveal={
                ...
                resolvedDiscardsSettledInHand < (gameplayRenderState.crib?.length ?? 0)
              }
              dealerDisplayName={
