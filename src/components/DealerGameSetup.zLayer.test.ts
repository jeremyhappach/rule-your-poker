...
describe('DealerGameSetup canonical modal layering', () => {
  it('places every setup surface above the high-card reveal layer', () => {
    ...
    // The three existing surfaces plus the isolated Farkle development form.
    // Every surface keeps the same canonical modal layer above the reveal.
    expect(source.match(/style=\{\{ zIndex: SHELL_Z\.MODAL_OVERLAY \}\}/g)).toHaveLength(4);
    expect(source).toContain('data-dealer-game-setup-selected-game="farkle"');
  });
