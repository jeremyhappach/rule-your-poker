/**
 * Wave 5D — Farkle anchored slot (generic).
 *
 * Mirrors GinAnchoredSlot. Pure consumer of a single anchored
 * placement from FarkleGameplayGeometryProvider. Renders children
 * inside an absolutely-positioned div whose rect is the assigned
 * anchored rect in felt-local vmin. Installs the standard DOM-bounds
 * contract and center-drift assertion.
 */
...
import { useFarkleGameplayGeometry } from "@/lib/farkle/FarkleGameplayGeometryProvider";
...
export interface FarkleAnchoredSlotProps {
  artifactId: string;
  zIndex?: number;
...
export function FarkleAnchoredSlot({
  ...
  return createPortal(
    <div
      ref={ref}
      data-wave5-farkle-slot={artifactId}
      ...
      data-farkle-slot-fault-count={String(faults.length)}
      style={{
        position: "absolute",
        left: `${x}vmin`,
        top: `${y}vmin`,
...
export default FarkleAnchoredSlot;
