...
export function YahtzeeGameTable({
  ...
  const [cachedOpponentDice, setCachedOpponentDice] = useState<{ dice: HorsesDieType[]; rollKey?: string; playerId: string } | null>(null);
  // Always track last non-zero dice for current turn player (used to cache for scoring transition)
  const lastNonZeroDiceRef = useRef<{ dice: HorsesDieType[]; rollKey?: string; playerId: string } | null>(null);
