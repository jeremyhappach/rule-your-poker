...
if (!runNamespace || !/^[a-z0-9][a-z0-9_-]{0,63}$/i.test(runNamespace)) {
  throw new Error('Target gauntlet requires a unique PTOWN_E2E_RUN_NAMESPACE.');
}

const outputRoot = process.env.PTOWN_E2E_OUTPUT_DIR?.trim() || 'test-results/target-gauntlet';
const outputDir = path.join(outputRoot, runNamespace);
const reportDir = path.join('playwright-report/target-gauntlet', runNamespace);
...
export default defineConfig({
  testDir: './e2e/targetGauntlet',
  testMatch: '**/*.targetGauntlet.spec.ts',
  fullyParallel: false,
  workers: 1,
  timeout: 8 * 60_000,
