...
BEGIN
  ...
  FOR v_case IN 0..6 LOOP
    BEGIN
      ...
      ELSIF v_case=6 THEN UPDATE public.players SET stand_up_next_hand=true WHERE id=v_player_one;
      END IF;
      ...
      IF v_result->>'outcome'<>'advanced' OR v_result->>'status'<>v_expected
         ...
         OR current_setting('app.gin_rummy_authoritative_write',true)<>'' THEN
      ...
      SELECT user_id INTO v_actor FROM public.players
