CREATE OR REPLACE FUNCTION private.replay_gin_predecessor_prepare(_category text,_harness text,_bot boolean DEFAULT false)
...
BEGIN
 ...
 INSERT INTO public.games(id,name,game_type,status,ante_amount,buy_in,pot,total_hands,points_to_win,current_host,dealer_position,replay_contract_version,pending_session_end)
  VALUES(g,'Gin replay commit benchmark','gin-rummy','ante_decision',1,1000,0,0,v_points,u1,1,CASE WHEN _enabled THEN 1 END,_category='settlement_terminal');
...
END;
$fn$;
REVOKE ALL ON FUNCTION private.replay_gin_predecessor_prepare(text,text,boolean) FROM PUBLIC,anon,authenticated,service_role;
