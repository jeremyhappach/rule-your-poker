-- Temporary observation expires 2026-09-17 12:00 UTC. No replay contract or gameplay changes.
...
DO $guard$ BEGIN IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='private' AND p.proname='replay_gin_open_v1' AND md5(pg_get_functiondef(p.oid))='eedb9c98f8f292d40008bc83c1ff5200') THEN RAISE EXCEPTION 'live_timing:owner_drift:replay_gin_open_v1'; END IF; END $guard$;
...
DO $guard$ BEGIN IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='private' AND p.proname='replay_gin_shared_begin_v1' AND md5(pg_get_functiondef(p.oid))='ff17d7d12fe36a26ffddc93e9c61012e') THEN RAISE EXCEPTION 'live_timing:owner_drift:replay_gin_shared_begin_v1'; END IF; END $guard$;
...
DO $guard$ BEGIN IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='private' AND p.proname='replay_gin_shared_end_v1' AND md5(pg_get_functiondef(p.oid))='96940e7ee9601da39ba0de1459c27db6') THEN RAISE EXCEPTION 'live_timing:owner_drift:replay_gin_shared_end_v1'; END IF; END $guard$;
...
DO $guard$ BEGIN IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='private' AND p.proname='replay_gin_transition_v1' AND md5(pg_get_functiondef(p.oid))='d21ee2d370b53f312bd6fbaa5945f7a5') THEN RAISE EXCEPTION 'live_timing:owner_drift:replay_gin_transition_v1'; END IF; END $guard$;
...
DO $guard$ BEGIN IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='private' AND p.proname='replay_gin_note_transfer_v1' AND md5(pg_get_functiondef(p.oid))='65a37974841d4232ffa47c555f3ce8fd') THEN RAISE EXCEPTION 'live_timing:owner_drift:replay_gin_note_transfer_v1'; END IF; END $guard$;
...
DO $guard$ BEGIN IF NOT EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='private' AND p.proname='replay_gin_postgame_v1' AND md5(pg_get_functiondef(p.oid))='dde5f272e081d34bbd2dde24c9dd9f24') THEN RAISE EXCEPTION 'live_timing:owner_drift:replay_gin_postgame_v1'; END IF; END $guard$;
