-- Real-money fixture exclusion and full Holm private-card/settlement proof. Real database roles and fake-money fixtures.
-- Synthetic sessions and transactional settings changes all roll back; historical sessions are untouched.
BEGIN;
...
DO $fixtures$
DECLARE
 f jsonb; g public.games; p1 uuid; p2 uuid; deck jsonb; base jsonb; actual jsonb;
 state jsonb; fake_state jsonb; row_state jsonb; profile text; mode boolean;
 cut_round uuid; pid uuid;
 baseline_cut jsonb; observed_kind text; expected text; result jsonb; before_replay jsonb;
BEGIN
 -- Global settings changes exist only inside this transaction and roll back.
 UPDATE public.system_settings SET value=jsonb_set(value,'{enabled}','true'::jsonb) WHERE key='harnesses_mode';
 f:=pg_temp.holm_boundary_fixture('fixture containment',false,false);
 SELECT * INTO g FROM public.games WHERE id=(f->>'game')::uuid;
 p1:=(f->>'p1')::uuid; p2:=(f->>'p2')::uuid;
 deck:=private.cribbage_new_deck();
 UPDATE public.game_defaults SET debug_harness='none' WHERE game_type='cribbage';
 base:=private.cribbage_initial_state(g,ARRAY[p1,p2],p2,deck);
 FOREACH profile IN ARRAY ARRAY['near_double_skunk','max_pegging_fan','perpetual_heels'] LOOP
   UPDATE public.game_defaults SET debug_harness=profile WHERE game_type='cribbage';
   g.real_money:=true;
   actual:=private.cribbage_initial_state(g,ARRAY[p1,p2],p2,deck);
   IF actual IS DISTINCT FROM base THEN RAISE EXCEPTION 'fixture:real_crib_deal_changed:%',profile; END IF;
   g.real_money:=NULL;
   actual:=private.cribbage_initial_state(g,ARRAY[p1,p2],p2,deck);
   IF actual IS DISTINCT FROM base THEN RAISE EXCEPTION 'fixture:unknown_crib_deal_changed:%',profile; END IF;
   g.real_money:=false;
   fake_state:=private.cribbage_initial_state(g,ARRAY[p1,p2],p2,deck);
   IF profile<>'perpetual_heels' AND fake_state IS NOT DISTINCT FROM base THEN
     RAISE EXCEPTION 'fixture:fake_crib_fixture_lost:%',profile;
   END IF;
 END LOOP;

 -- A persisted targeted profile cannot force a cut when the game is real.
 UPDATE public.game_defaults SET debug_harness='none' WHERE game_type='cribbage';
 UPDATE public.games SET real_money=true WHERE id=g.id;
 PERFORM setseed(0.37);
 baseline_cut:=private.cribbage_finish_discard(base,g.id)->'cutCard';
 UPDATE public.game_defaults SET debug_harness='perpetual_heels' WHERE game_type='cribbage';
 PERFORM setseed(0.37);
 actual:=private.cribbage_finish_discard(base||'{"campaignHarnessProfile":"perpetual_heels"}'::jsonb,g.id);
 IF actual->'cutCard' IS DISTINCT FROM baseline_cut THEN RAISE EXCEPTION 'fixture:real_crib_cut_changed'; END IF;
 UPDATE public.games SET real_money=false WHERE id=g.id;
 actual:=private.cribbage_finish_discard(base,g.id);
 IF actual->'cutCard'->>'rank'<>'J' THEN RAISE EXCEPTION 'fixture:fake_crib_cut_lost'; END IF;

 -- Seeded ordinary deals are unchanged by every legacy global Gin fixture.
 UPDATE public.game_defaults SET debug_harness='none' WHERE game_type='gin-rummy';
 g.real_money:=true;
 PERFORM setseed(0.61);
 base:=(private.gin_deal_state(g,p2,p1,'{}'::jsonb,1,100,1)-'botActionDueAt');
 FOREACH profile IN ARRAY ARRAY['near_gin','opponent_instant_knock','non_dealer_near_knock'] LOOP
   UPDATE public.game_defaults SET debug_harness=profile WHERE game_type='gin-rummy';
   PERFORM setseed(0.61);
   actual:=(private.gin_deal_state(g,p2,p1,'{}'::jsonb,1,100,1)-'botActionDueAt');
   IF actual IS DISTINCT FROM base THEN RAISE EXCEPTION 'fixture:real_gin_deal_changed:%',profile; END IF;
   g.real_money:=NULL;
   PERFORM setseed(0.61);
   actual:=(private.gin_deal_state(g,p2,p1,'{}'::jsonb,1,100,1)-'botActionDueAt');
   IF actual IS DISTINCT FROM base THEN RAISE EXCEPTION 'fixture:unknown_gin_deal_changed:%',profile; END IF;
   g.real_money:=false;
   PERFORM setseed(0.61);
   fake_state:=(private.gin_deal_state(g,p2,p1,'{}'::jsonb,1,100,1)-'botActionDueAt');
   IF fake_state IS NOT DISTINCT FROM base THEN RAISE EXCEPTION 'fixture:fake_gin_fixture_lost:%',profile; END IF;
   g.real_money:=true;
 END LOOP;

 -- Real authenticated Holm decisions ignore forced winners for real money;
 -- the same global setting remains executable for an explicit fake table.
 FOREACH mode IN ARRAY ARRAY[true,false] LOOP
   f:=pg_temp.holm_boundary_fixture('Holm fixture outcome '||mode,false,false);
   UPDATE public.games SET real_money=mode WHERE id=(f->>'game')::uuid;
   UPDATE public.game_defaults SET debug_harness='force_chucky_beats_player' WHERE game_type='holm';
   PERFORM set_config('request.jwt.claim.sub',f->>'u1',true);
   PERFORM set_config('request.jwt.claims',jsonb_build_object('sub',f->>'u1','role','authenticated')::text,true);
   EXECUTE 'SET LOCAL ROLE authenticated';
   PERFORM public.holm_submit_decision((f->>'game')::uuid,(f->>'round')::uuid,(f->>'p1')::uuid,'stay');
   EXECUTE 'RESET ROLE';
   PERFORM set_config('request.jwt.claim.sub',f->>'u2',true);
   PERFORM set_config('request.jwt.claims',jsonb_build_object('sub',f->>'u2','role','authenticated')::text,true);
   EXECUTE 'SET LOCAL ROLE authenticated';
   result:=public.holm_submit_decision((f->>'game')::uuid,(f->>'round')::uuid,(f->>'p2')::uuid,'fold');
   EXECUTE 'RESET ROLE';
   expected:=CASE WHEN mode THEN 'chucky_final_award' ELSE 'chucky_loss_pot_match' END;
   SELECT event_kind::text INTO observed_kind FROM public.game_results WHERE game_id=(f->>'game')::uuid AND event_kind IS NOT NULL;
   IF observed_kind IS DISTINCT FROM expected THEN RAISE EXCEPTION 'fixture:holm_outcome:%:%:%',mode,observed_kind,result; END IF;
   before_replay:=pg_temp.holm_boundary_snapshot((f->>'game')::uuid);
   PERFORM public.holm_submit_decision((f->>'game')::uuid,(f->>'round')::uuid,(f->>'p2')::uuid,'fold');
   IF pg_temp.holm_boundary_snapshot((f->>'game')::uuid) IS DISTINCT FROM before_replay THEN RAISE EXCEPTION 'fixture:holm_replay_changed'; END IF;
   IF (SELECT sum(chips) FROM public.players WHERE game_id=(f->>'game')::uuid)+(SELECT pot FROM public.games WHERE id=(f->>'game')::uuid)<>200 THEN
     RAISE EXCEPTION 'fixture:holm_conservation';
   END IF;
 END LOOP;

 -- Both public discard owners pass the real game identity to the private cut.
 f:=pg_temp.holm_boundary_fixture('Crib discard owners',false,false);
 SELECT * INTO g FROM public.games WHERE id=(f->>'game')::uuid;
 p1:=(f->>'p1')::uuid; p2:=(f->>'p2')::uuid;
 PERFORM set_config('app.cribbage_authoritative_write','on',true);
 INSERT INTO public.dealer_games(id,session_id,dealer_user_id,game_type)
 VALUES(gen_random_uuid(),g.id,(f->>'u1')::uuid,'cribbage') RETURNING id INTO g.current_game_uuid;
 UPDATE public.games SET current_game_uuid=g.current_game_uuid,game_type='cribbage',real_money=true WHERE id=g.id;
 INSERT INTO public.rounds(game_id,dealer_game_id,hand_number,round_number,cards_dealt,pot,status)
 VALUES(g.id,g.current_game_uuid,2,1,6,0,'betting') RETURNING id INTO cut_round;
 UPDATE public.game_defaults SET debug_harness='none' WHERE game_type='cribbage';
 g.real_money:=true;
 state:=private.cribbage_initial_state(g,ARRAY[p1,p2],p2,private.cribbage_new_deck());
 PERFORM private.cribbage_publish_state(cut_round,state);
 PERFORM set_config('request.jwt.claim.sub',f->>'u1',true);
 PERFORM set_config('request.jwt.claims',jsonb_build_object('sub',f->>'u1','role','authenticated')::text,true);
 EXECUTE 'SET LOCAL ROLE authenticated';
 PERFORM public.cribbage_apply_discard(cut_round,p1,ARRAY[0,1]);
 EXECUTE 'RESET ROLE';
 PERFORM set_config('request.jwt.claim.sub',f->>'u2',true);
 PERFORM set_config('request.jwt.claims',jsonb_build_object('sub',f->>'u2','role','authenticated')::text,true);
 EXECUTE 'SET LOCAL ROLE authenticated';
 PERFORM public.cribbage_apply_discard(cut_round,p2,ARRAY[0,1]);
 EXECUTE 'RESET ROLE';
 SELECT s.state INTO actual FROM private.cribbage_round_states s WHERE round_id=cut_round;
 IF actual->>'phase'<>'pegging' OR actual->'cutCard'='null'::jsonb THEN RAISE EXCEPTION 'fixture:discard_owner_cut_missing'; END IF;
 -- Reconstruct the all-discarded checkpoint without its cut or any heels score.
 row_state:=state;
 FOREACH pid IN ARRAY ARRAY[p1,p2] LOOP
   row_state:=jsonb_set(row_state,ARRAY['playerStates',pid::text,'discardedToCrib'],jsonb_build_array(state->'playerStates'->pid::text->'hand'->0,state->'playerStates'->pid::text->'hand'->1));
   row_state:=jsonb_set(row_state,ARRAY['playerStates',pid::text,'hand'],(state->'playerStates'->pid::text->'hand')-0-0);
 END LOOP;
 row_state:=jsonb_set(row_state,'{crib}',(row_state->'playerStates'->p1::text->'discardedToCrib')||(row_state->'playerStates'->p2::text->'discardedToCrib'));
 PERFORM private.cribbage_publish_state(cut_round,row_state);
 EXECUTE 'SET LOCAL ROLE authenticated';
 PERFORM public.cribbage_reconcile_discard_transition(cut_round);
 EXECUTE 'RESET ROLE';
 SELECT s.state INTO actual FROM private.cribbage_round_states s WHERE round_id=cut_round;
 IF actual->>'phase'<>'pegging' OR actual->'cutCard'='null'::jsonb THEN RAISE EXCEPTION 'fixture:reconcile_owner_cut_missing'; END IF;
