...
BEGIN
  ...
  -- Each scenario rolls back to the identical server-settled game. These calls
  -- use the real authenticated database role, with no browser cleanup beforehand.
  FOR v_case IN 0..6 LOOP
    BEGIN
      ...
      UPDATE public.players SET status='active',sitting_out=false,waiting=false,
        stand_up_next_hand=false,sit_out_next_hand=false,auto_fold=true
       WHERE game_id=v_game_id;
      UPDATE public.games SET pending_session_end=(v_case=5) WHERE id=v_game_id;
      IF v_case=1 THEN UPDATE public.players SET sit_out_next_hand=true WHERE id=v_player_two;
      ELSIF v_case=2 THEN UPDATE public.players SET stand_up_next_hand=true WHERE id=v_player_two;
      ELSIF v_case=3 THEN UPDATE public.players SET waiting=true,sitting_out=true WHERE id=v_player_two;
      ELSIF v_case=4 THEN UPDATE public.players SET sit_out_next_hand=true WHERE game_id=v_game_id;
      ELSIF v_case=6 THEN UPDATE public.players SET stand_up_next_hand=true WHERE id=v_winner_id;
      END IF;
      SELECT sum(chips) INTO v_money FROM public.players WHERE game_id=v_game_id;
      v_expected:=CASE WHEN v_case IN(4,5) THEN 'session_ended' WHEN v_case IN(1,2,6) THEN 'waiting' ELSE 'game_selection' END;
      PERFORM set_config('app.cribbage_authoritative_write','',true);
      SET LOCAL ROLE authenticated;
      v_result:=public.cribbage_advance_postgame(v_game_id,v_next_round_id,v_dealer_game_id,2);
      RESET ROLE;
      IF v_result->>'outcome'<>'advanced' OR v_result->>'status'<>v_expected
         OR (SELECT status FROM public.games WHERE id=v_game_id)<>v_expected
         OR (SELECT sum(chips) FROM public.players WHERE game_id=v_game_id) IS DISTINCT FROM v_money
         OR (SELECT count(*) FROM public.players WHERE game_id=v_game_id)<>2
         OR EXISTS(SELECT 1 FROM public.players WHERE game_id=v_game_id AND
           (auto_fold OR waiting OR stand_up_next_hand OR sit_out_next_hand))
         OR current_setting('app.cribbage_authoritative_write',true)<>'' THEN
        RAISE EXCEPTION 'atomic_postgame_case_failed:%:%',v_case,v_result;
      END IF;
      IF v_case=2 AND (SELECT status FROM public.players WHERE id=v_player_two)<>'left' THEN
        RAISE EXCEPTION 'queued_departure_not_retained';
      END IF;
      IF v_case=3 AND (SELECT sitting_out FROM public.players WHERE id=v_player_two) THEN
        RAISE EXCEPTION 'waiting_participant_not_reactivated';
      END IF;
      IF v_case=0 AND EXISTS(SELECT 1 FROM public.players WHERE game_id=v_game_id AND sitting_out) THEN
        RAISE EXCEPTION 'stale_autofold_forced_sitout';
      END IF;
      IF v_case=5 AND ((SELECT pending_session_end FROM public.games WHERE id=v_game_id)
        OR (SELECT session_ended_at FROM public.games WHERE id=v_game_id) IS NULL) THEN
        RAISE EXCEPTION 'pending_terminal_disposition_not_committed';
      END IF;
      SELECT jsonb_agg(to_jsonb(p) ORDER BY p.id) INTO v_participants FROM public.players p WHERE game_id=v_game_id;
      -- A surviving member can retry even if the winner queued departure.
      SELECT user_id INTO v_current_player FROM public.players
       WHERE game_id=v_game_id AND status<>'left' ORDER BY position LIMIT 1;
      PERFORM set_config('request.jwt.claims',jsonb_build_object('sub',v_current_player,'role','authenticated')::text,true);
      SET LOCAL ROLE authenticated;
      ...
      IF v_replay->>'outcome'<>'already_advanced'
         OR (SELECT jsonb_agg(to_jsonb(p) ORDER BY p.id) FROM public.players p WHERE game_id=v_game_id) IS DISTINCT FROM v_participants THEN
        RAISE EXCEPTION 'atomic_postgame_replay_changed_participation:%',v_case;
      END IF;
      RAISE EXCEPTION USING ERRCODE='Z0001',MESSAGE='rollback successful scenario';
