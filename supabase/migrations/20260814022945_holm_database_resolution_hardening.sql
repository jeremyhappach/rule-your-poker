...
BEGIN
  ...
  IF position(v_after IN v_definition)=0 THEN
    IF position(v_before IN v_definition)=0 THEN RAISE EXCEPTION 'holm_database_resolution_hardening:settlement_boundary_not_found'; END IF;
...
DECLARE
  ...
  v_after text := E'  IF coalesce((v_result->>''all_decisions_in'')::boolean, false) THEN
    ...
    UPDATE public.rounds';
BEGIN
  SELECT pg_get_functiondef('public.holm_submit_decision(uuid,uuid,uuid,text)'::regprocedure) INTO v_definition;
  IF position('public.resolve_holm_showdown(p_game_id, v_round.id)' IN v_definition)=0 THEN
    IF position(v_before IN v_definition)=0 THEN RAISE EXCEPTION 'holm_database_resolution_hardening:decision_boundary_not_found'; END IF;
    EXECUTE replace(v_definition,v_before,v_after);
  END IF;
END; $migration$;

CREATE OR REPLACE FUNCTION public.recover_pending_holm_showdowns(p_game_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public
AS $$
DECLARE v_candidate record; v_result jsonb; v_resolved integer:=0; v_rejected integer:=0;
BEGIN
  IF coalesce(auth.jwt()->>'role','')<>'service_role' THEN RAISE EXCEPTION 'recover_pending_holm_showdowns:service_role_required'; END IF;
  FOR v_candidate IN
    SELECT game.id AS game_id,round.id AS round_id FROM public.games game JOIN public.rounds round ON round.game_id=game.id AND round.dealer_game_id IS NOT DISTINCT FROM game.current_game_uuid
    WHERE game.id=p_game_id AND game.game_type IN ('holm','holm-game') AND game.status NOT IN ('game_over','session_ended') AND coalesce(game.is_paused,false)=false AND round.status IN ('betting','processing','showdown')
      AND EXISTS (SELECT 1 FROM public.players participant WHERE participant.game_id=game.id AND participant.status='active' AND participant.sitting_out=false)
      AND NOT EXISTS (SELECT 1 FROM public.players participant WHERE participant.game_id=game.id AND participant.status='active' AND participant.sitting_out=false AND (NOT participant.decision_locked OR participant.current_decision NOT IN ('stay','fold')))
    ORDER BY game.id,round.id LIMIT 100
  LOOP
    SELECT public.resolve_holm_showdown(v_candidate.game_id,v_candidate.round_id) INTO v_result;
    IF v_result->>'outcome' IN ('resolved','already-resolved') THEN v_resolved:=v_resolved+1; ELSE v_rejected:=v_rejected+1; END IF;
  END LOOP;
