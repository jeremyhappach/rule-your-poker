-- Farkle-only turn-clock correction. Historical authority migrations remain unchanged.
DO $guard$
BEGIN
  PERFORM pg_advisory_xact_lock(19092026,1);
  IF md5(pg_get_functiondef('public.farkle_apply_action(uuid,uuid,text,bigint,uuid,jsonb)'::regprocedure)) <> '44e9c9b5dbda2c5415fbdec19d2d3289'
  THEN RAISE EXCEPTION 'farkle_turn_clock:action_owner_drift'; END IF;
  ...
  THEN RAISE EXCEPTION 'farkle_turn_clock:release_gate_drift'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.game_defaults WHERE game_type='farkle' AND decision_timer_seconds=10)
  THEN RAISE EXCEPTION 'farkle_turn_clock:default_drift'; END IF;
END $guard$;
...
BEGIN
 ...
 IF s->>'gamePhase'='playing' THEN
  ...
  -- Ordinary human actions retain the authoritative turn deadline. A new actor
  -- starts a new turn; an authoritative Hot Dice Hold renews that human turn.
  -- Bot-controlled actions continue to use their separate action pacing.
  IF s->>'currentTurnPlayerId' IS DISTINCT FROM p.id::text
    OR next_actor.is_bot OR next_actor.auto_fold
    OR (p_action='hold' AND EXISTS (
      SELECT 1 FROM jsonb_array_elements(s->'events') event WHERE event->>'type'='hot_dice')) THEN
...
END $function$
;
UPDATE public.game_defaults SET decision_timer_seconds=60
WHERE game_type='farkle' AND decision_timer_seconds=10;
DO $verify$
BEGIN
  IF (SELECT decision_timer_seconds FROM public.game_defaults WHERE game_type='farkle') IS DISTINCT FROM 60
  THEN RAISE EXCEPTION 'farkle_turn_clock:default_not_updated'; END IF;
END $verify$;
