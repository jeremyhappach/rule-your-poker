-- Holm terminal settlement: extend the single authoritative server operation so it
-- owns payout + result + post-payout snapshot + terminal disposition atomically.
-- The previous signature is unused by live code; drop and recreate to extend params.
DROP FUNCTION IF EXISTS public.holm_settle_hand(uuid, uuid, integer, holm_event_kind, integer, boolean, text, jsonb, text, uuid, text, boolean, integer);

CREATE OR REPLACE FUNCTION public.holm_settle_hand(
  p_game_id uuid,
  p_dealer_game_id uuid,
  p_hand_number integer,
  p_event_kind holm_event_kind,
  p_pot_final integer,
  p_awaiting_next_round boolean,
  p_last_round_result text,
  p_chip_deltas jsonb,
  ...
  p_pot_won integer,
  p_mark_round_completed boolean DEFAULT true,
  p_round_pot integer DEFAULT NULL,
  p_clear_chucky_active boolean DEFAULT false,
  p_reset_player_states boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_round.status NOT IN ('completed','in_progress','showdown','revealing','betting','dealing') THEN
        ...
        IF v_end_session THEN
          ...
          IF NOT v_snap_exists THEN
            INSERT INTO public.session_player_snapshots
              (game_id, player_id, user_id, username, chips, is_bot, hand_number)
            SELECT p.game_id, p.id, p.user_id,
  ...
  IF v_end_game THEN
    ...
    -- fires record_session_results. Idempotent on (game_id, hand_number).
    IF v_end_session THEN
      SELECT EXISTS (
        SELECT 1 FROM public.session_player_snapshots
         WHERE game_id = p_game_id AND hand_number = p_hand_number
...
GRANT EXECUTE ON FUNCTION public.holm_settle_hand(
  uuid, uuid, integer, holm_event_kind, integer, boolean, text, jsonb, text, uuid, text, boolean, integer,
