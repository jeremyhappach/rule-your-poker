-- Update horses_advance_turn to atomically set turnDeadline when advancing turns
-- This prevents the race condition where client fails to set deadline after advancing
...
BEGIN
  ...
  ELSE
    ...
    -- Set deadline: 10 seconds for humans, NULL for bots (bots are handled by controller)
    IF next_player_is_bot THEN
      new_deadline := NULL;
    ELSE
      new_deadline := to_char((now() + interval '10 seconds') AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
    END IF;
