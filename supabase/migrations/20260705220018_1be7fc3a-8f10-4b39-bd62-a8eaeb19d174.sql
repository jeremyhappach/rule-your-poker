...
ALTER TABLE public.chat_send_operations
  ...
  ADD COLUMN IF NOT EXISTS delivery_confirmed_kind text,
  ADD COLUMN IF NOT EXISTS observation_window_start_at timestamptz,
  ADD COLUMN IF NOT EXISTS observation_window_end_at timestamptz;
...
BEGIN
  ...
  UPDATE public.chat_send_operations
     SET delivery_confirmed_at = COALESCE(delivery_confirmed_at, now()),
         delivery_confirmed_kind = COALESCE(delivery_confirmed_kind, _kind),
         observation_window_start_at = COALESCE(observation_window_start_at, now()),
         boundary_events = COALESCE(boundary_events, '[]'::jsonb) || _entry,
         updated_at = now()
...
GRANT EXECUTE ON FUNCTION public.chat_operation_mark_delivery_confirmed(text, text, jsonb) TO authenticated;

-- Rewrite finalizer to stamp observation_window_end_at + include new fields in report
...
DECLARE
  ...
  _armed_after_receipt boolean;
  _first_missing_sender_hb text;
BEGIN
  ...
  -- Instrumentation-armed heuristic: any boundary event or sender/peer heartbeat AFTER delivery_confirmed_at.
  _armed_after_receipt := (
    _row.delivery_confirmed_at IS NOT NULL AND (
      (_row.last_sender_heartbeat_at IS NOT NULL AND _row.last_sender_heartbeat_at > _row.delivery_confirmed_at) OR
      (_row.last_peer_heartbeat_at   IS NOT NULL AND _row.last_peer_heartbeat_at   > _row.delivery_confirmed_at) OR
      EXISTS (
        SELECT 1 FROM jsonb_array_elements(COALESCE(_row.boundary_events, '[]'::jsonb)) AS e
        WHERE (e->>'at')::timestamptz > _row.delivery_confirmed_at
      )
    )
  );

  -- First observed gap in sender heartbeats after delivery, in seconds (best-effort text).
  IF _row.delivery_confirmed_at IS NOT NULL
     AND _row.last_sender_heartbeat_at IS NOT NULL
     AND _row.last_sender_heartbeat_at > _row.delivery_confirmed_at
  THEN
    _first_missing_sender_hb :=
      'last_sender_heartbeat_at ' || _row.last_sender_heartbeat_at::text
      || ' vs finalize ' || now()::text;
  ELSIF _row.delivery_confirmed_at IS NOT NULL THEN
    _first_missing_sender_hb := 'no sender heartbeat after delivery_confirmed_at';
  ELSE
    _first_missing_sender_hb := '(delivery not confirmed)';
  END IF;
  ...
  UPDATE public.chat_send_operations
     SET status = 'finalized',
         ...
         observation_window_end_at = COALESCE(observation_window_end_at, now()),
  ...
  _report_json := jsonb_build_object(
    ...
    'presence', jsonb_build_object(
      ...
      'delivery_confirmed_at', _row.delivery_confirmed_at,
      'delivery_confirmed_kind', _row.delivery_confirmed_kind,
      'observation_window_start_at', _row.observation_window_start_at,
      'observation_window_end_at', _row.observation_window_end_at,
      'instrumentation_armed_after_receipt', _armed_after_receipt,
      'first_missing_sender_heartbeat', _first_missing_sender_hb
    ),
  ...
  _report_text :=
    ...
    E'\n' ||
    'PRESENCE' || E'\n' ||
    'started_at                  : ' || COALESCE(_row.started_at::text, '(null)') || E'\n' ||
    'delivery_confirmed_at       : ' || COALESCE(_row.delivery_confirmed_at::text, '(null)') || E'\n' ||
    'delivery_confirmed_kind     : ' || COALESCE(_row.delivery_confirmed_kind, '(null)') || E'\n' ||
    'observation_window_start_at : ' || COALESCE(_row.observation_window_start_at::text, '(null)') || E'\n' ||
    'observation_window_end_at   : ' || COALESCE(_row.observation_window_end_at::text, '(null)') || E'\n' ||
    'instrumentation_armed_after : ' || CASE WHEN _armed_after_receipt THEN 'yes' ELSE 'NO' END || E'\n' ||
    'first_missing_sender_hb     : ' || COALESCE(_first_missing_sender_hb, '(null)') || E'\n' ||
