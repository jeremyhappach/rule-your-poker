...
BEGIN
  ...
  SELECT array_agg(id) INTO v_round_ids FROM public.rounds WHERE game_id = ANY(v_game_ids);
  ...
  IF v_round_ids IS NOT NULL THEN
    ...
    UPDATE public.rounds SET predecessor_round_id = NULL WHERE game_id = ANY(v_game_ids);
  END IF;
