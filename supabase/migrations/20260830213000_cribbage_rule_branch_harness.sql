-- Exact-game, fake-money-only, one-shot Cribbage rule-branch fixtures.
--
-- These fixtures expose the three existing authoritative Cribbage debug
-- profiles without changing the global harness gate. Requests are scoped to
-- one generated session, may be armed only by an authenticated admin who is
-- already a participant, expire quickly, and are consumed atomically by the
-- first authoritative hand.

INSERT INTO public.system_settings (key, value)
VALUES ('cribbage_rule_branch_harness', jsonb_build_object('requests', '{}'::jsonb))
ON CONFLICT (key) DO NOTHING;

CREATE OR REPLACE FUNCTION public.get_cribbage_rule_branch_harness(p_game_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = ''
...
CREATE OR REPLACE FUNCTION public.cancel_cribbage_rule_branch_harness(p_game_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = ''
...
REVOKE ALL ON FUNCTION public.get_cribbage_rule_branch_harness(uuid)
  FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.arm_cribbage_rule_branch_harness(uuid,text,integer)
  FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.cancel_cribbage_rule_branch_harness(uuid)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_cribbage_rule_branch_harness(uuid)
  TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.arm_cribbage_rule_branch_harness(uuid,text,integer)
  TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.cancel_cribbage_rule_branch_harness(uuid)
  TO authenticated, service_role;
...
BEGIN
  ...
  IF NOT coalesce((v_request->>'armed')::boolean, false)
     ...
     OR v_profile NOT IN ('near_double_skunk', 'max_pegging_fan', 'perpetual_heels')
...
AS $$
DECLARE
  v_state jsonb := p_state - 'pendingTerminal' - 'campaignHarnessProfile';
  v_player_id text;
...
BEGIN
  ...
  FOR v_player_id IN SELECT jsonb_object_keys(coalesce(v_state->'playerStates', '{}'::jsonb)) LOOP
    ...
    v_player_state := jsonb_set(v_player_state, '{hand}', private.cribbage_mask_cards(v_player_state->'hand'), true);
    IF NOT v_reveal THEN
      v_player_state := jsonb_set(v_player_state, '{discardedToCrib}', private.cribbage_mask_cards(v_player_state->'discardedToCrib'), true);
    END IF;
...
BEGIN
  ...
  IF coalesce(v_harness_enabled, false)
    ...
    -- The global max-pegging profile keeps its exact deck. The exact-game
    -- His Heels profile uses the same jack-free opening cohort so a jack is
    -- guaranteed to remain available for the authoritative cut; the global
    -- His Heels profile remains byte-for-byte unchanged.
