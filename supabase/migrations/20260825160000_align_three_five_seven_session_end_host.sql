-- A live dealer game may retain the legacy null current_host value. The client
-- already resolves that identity to the earliest seated human, so LAST HAND
-- must authorize the same person instead of rejecting the visible host action.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'three_five_seven_request_session_end:not_active_human_participant';
  END IF;

  SELECT participant.user_id
    INTO v_fallback_host_id
    FROM public.players participant
   WHERE participant.game_id = p_game_id
  ...
  IF coalesce(v_game.current_host, v_fallback_host_id) IS DISTINCT FROM v_actor_id THEN
    RAISE EXCEPTION 'three_five_seven_request_session_end:not_session_host';
  END IF;
