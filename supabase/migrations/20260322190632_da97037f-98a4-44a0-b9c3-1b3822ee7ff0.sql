-- Update horses_advance_turn to read decision_timer_seconds from game_defaults
-- instead of hardcoding 30 seconds
...
BEGIN
  ...
  -- Read game_type to look up the correct timer default
  SELECT g.game_type INTO g_game_type
  FROM public.games g
  WHERE g.id = g_id;

  -- Read configurable timer from game_defaults (fallback to 30 seconds)
  SELECT gd.decision_timer_seconds INTO timer_seconds
  FROM public.game_defaults gd
  WHERE gd.game_type = COALESCE(g_game_type, 'horses');

  IF timer_seconds IS NULL THEN
    timer_seconds := 30;
  END IF;

  -- Atomically set the current player's isComplete to true BEFORE advancing
  ...
  -- CRITICAL FIX: Always reset the NEXT player's state at the moment we advance.
  IF next_id IS NOT NULL THEN
    hs := jsonb_set(
      ...
      jsonb_build_object(
        ...
        'isComplete', false,
        'result', NULL,
        'heldMaskBeforeComplete', NULL,
        'heldCountBeforeComplete', NULL
      ),
      true
    );
  END IF;
  ...
  ELSE
    ...
    SELECT p.is_bot INTO next_player_is_bot
    FROM public.players p
    WHERE p.id = next_id::uuid
      AND p.game_id = g_id;

    -- Set deadline using configurable timer_seconds for humans, NULL for bots
    IF next_player_is_bot THEN
      new_deadline := NULL;
    ELSE
      new_deadline := to_char((now() + (timer_seconds || ' seconds')::interval) AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
    END IF;

    hs := hs || jsonb_build_object(
      'currentTurnPlayerId', next_id,
      'gamePhase', 'playing',
      'turnDeadline', new_deadline
    );
  END IF;
