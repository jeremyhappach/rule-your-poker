-- Preserve exact Yahtzee settlement replay after ordinary lifecycle progression.
--
...
BEGIN
  ...
  END IF;
  IF COALESCE(v_round.pot, 0) <> 0 THEN
  ...
  IF v_winner_count > 1 THEN
    ...
    END IF;
    IF v_game.ante_amount IS DISTINCT FROM v_ante_amount
       OR COALESCE(v_game.pot, 0) <> 0 THEN
      RAISE EXCEPTION 'yahtzee_settle_game:mutable_game_financial_mismatch';
    END IF;
  ...
  IF FOUND THEN
    IF v_existing_result.winner_player_id IS DISTINCT FROM v_winner_id
       ...
       OR v_round.status IS DISTINCT FROM 'completed' THEN
