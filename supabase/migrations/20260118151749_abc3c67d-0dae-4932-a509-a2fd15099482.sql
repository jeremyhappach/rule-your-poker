-- Fix: prevent stale dice flashing when advancing turns (especially bot->bot)
-- by ALWAYS resetting the next player's dice state to an unrolled hand.
...
BEGIN
  ...
  -- Include isSCC=false so SCC games also get a consistent die shape.
  ...
  -- This prevents clients from briefly rendering stale dice values from a previous round/turn.
  ...
  ELSE
    ...
    -- Set deadline: 30 seconds for humans, NULL for bots (bots are handled by controller)
    IF next_player_is_bot THEN
      new_deadline := NULL;
    ELSE
      new_deadline := to_char((now() + interval '30 seconds') AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
    END IF;
