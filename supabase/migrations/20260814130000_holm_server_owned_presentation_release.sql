-- Holm continuation publication is server-owned. Settlement prepares one
-- exact non-actionable successor; a database cron lease publishes it without
-- any browser acknowledgement. Connected clients remain free to finish the
-- predecessor presentation locally before rendering the published hand.
...
BEGIN
  ...
  IF FOUND THEN
    RETURN jsonb_build_object(
      ...
      'presentation_release_at', v_existing_round.presentation_fallback_at,
  ...
  -- The longest ordinary continuing Holm presentation is the result plate,
  -- canonical pot award, and replacement-pot flight. This durable release
  -- lease is server-owned; slower clients keep their own presentation hold.
  v_release_at := clock_timestamp() + interval '9 seconds';
  ...
  INSERT INTO public.rounds (
    ...
    '[]'::jsonb, 0, NULL,
    v_new_buck_position, 0, v_release_at,
  ...
  RETURN jsonb_build_object(
    ...
    'presentation_release_at', v_release_at,
    'presentation_fallback_at', v_release_at,
    'deduped', false
  );
END;
$$;
...
BEGIN
  ...
  IF NOT p_from_fallback THEN
    RETURN jsonb_build_object('outcome', 'rejected', 'reason', 'server-release-required');
  END IF;
  ...
  IF v_successor.presentation_fallback_at IS NULL
     OR v_successor.presentation_fallback_at > clock_timestamp() THEN
    RETURN jsonb_build_object(
      'outcome', 'rejected',
      'reason', 'presentation-release-not-due',
      'presentation_release_at', v_successor.presentation_fallback_at
    );
  END IF;
  ...
  FOR v_due IN
    SELECT successor.game_id,
     ...
     WHERE successor.status = 'dealing'
       ...
       AND successor.presentation_fallback_at <= clock_timestamp()
...
CREATE OR REPLACE FUNCTION private.clear_holm_buck_presentation_on_dealer_game_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public, private
AS $$
BEGIN
  IF NEW.game_type IN ('holm', 'holm-game')
     AND NEW.current_game_uuid IS DISTINCT FROM OLD.current_game_uuid THEN
    NEW.buck_transfer_presentation := NULL;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS clear_holm_buck_presentation_on_dealer_game_change ON public.games;
CREATE TRIGGER clear_holm_buck_presentation_on_dealer_game_change
BEFORE UPDATE OF current_game_uuid ON public.games
FOR EACH ROW
EXECUTE FUNCTION private.clear_holm_buck_presentation_on_dealer_game_change();
...
REVOKE ALL ON FUNCTION private.clear_holm_buck_presentation_on_dealer_game_change() FROM PUBLIC, anon, authenticated;

COMMENT ON FUNCTION public.activate_prepared_holm_hand(uuid, uuid, uuid, boolean) IS
  'Server-only exact Holm successor publication after the durable presentation release lease. Browsers cannot activate gameplay.';
COMMENT ON FUNCTION private.release_due_holm_presentations() IS
  'Database-scheduled Holm presentation release owner. Publishes each due prepared successor exactly once without a connected client.';
