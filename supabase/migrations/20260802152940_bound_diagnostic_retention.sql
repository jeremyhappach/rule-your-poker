-- Keep production diagnostics useful without allowing forensic tables to
-- consume the owned project's database quota indefinitely. Gameplay,
-- financial, audit, and session-history tables are intentionally untouched.
...
BEGIN
  ...
  GET DIAGNOSTICS _deleted = ROW_COUNT; _total := _total + _deleted;
  DELETE FROM public.insert_audit_log WHERE created_at < _cutoff;
  GET DIAGNOSTICS _deleted = ROW_COUNT; _total := _total + _deleted;
  ...
  GET DIAGNOSTICS _deleted = ROW_COUNT; _total := _total + _deleted;
  DELETE FROM public.session_events WHERE created_at < _cutoff;
  GET DIAGNOSTICS _deleted = ROW_COUNT; _total := _total + _deleted;
...
BEGIN
  ...
  PERFORM cron.schedule(
    ...
    '15 4 * * *',
    $cron$SELECT public.purge_expired_diagnostics(interval '7 days');$cron$
  );
END
$schedule$;
