-- Atomic, permissioned helpers for Horses state updates (prevents turn flip-flop from stale whole-JSON overwrites)

CREATE OR REPLACE FUNCTION public.horses_set_player_state(
  _round_id uuid,
  _player_id uuid,
  _state jsonb
)
...
CREATE OR REPLACE FUNCTION public.horses_advance_turn(
  _round_id uuid,
  _expected_current_player_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
