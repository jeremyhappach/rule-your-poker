-- Yahtzee has one decision deadline for the entire three-roll turn. A fake-money
-- human expiry completes that exact turn atomically and marks the player to sit
-- out next hand; a real-money expiry remains owned by the existing pause helper.
...
BEGIN
  ...
  IF NOT coalesce(v_player.is_bot,false) THEN
    -- A timeout creates dice auto-roll behavior for this human only; it never
    -- changes the human into a bot identity.
...
REVOKE ALL ON FUNCTION private.complete_due_fake_money_yahtzee_turn(uuid,uuid,integer)
  FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION private.complete_due_fake_money_yahtzee_turn(uuid,uuid,integer)
  TO service_role;
...
BEGIN
  ...
  LOOP
    ...
    END IF;
    IF v_result->>'outcome' IN ('completed','paused') THEN v_advanced:=v_advanced+1; END IF;
  END LOOP;
  FOR v_candidate IN
...
COMMENT ON FUNCTION private.complete_due_fake_money_yahtzee_turn(uuid,uuid,integer) IS
  'Exact-identity fake-money Yahtzee timeout owner. Marks a timed-out human auto-roll/sit-out-next-hand and completes that entire turn atomically.';
COMMENT ON FUNCTION private.advance_due_yahtzee_state() IS
  'Complete scheduled Yahtzee bootstrap, whole-turn timeout ownership, terminal settlement, and postgame recovery owner.';
