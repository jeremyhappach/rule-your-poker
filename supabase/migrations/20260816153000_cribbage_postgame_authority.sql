-- Move the Cribbage terminal dealer-game handoff behind one replay-safe
-- database owner. Browser clients may request this transition only after the
-- exact terminal round has settled; they no longer derive or write the next
-- dealer and lifecycle state themselves.

CREATE TABLE IF NOT EXISTS private.cribbage_postgame_advances (
  ...
  target_status text NOT NULL CHECK (target_status IN ('dealer_selection', 'game_selection')),
...
ALTER TABLE private.cribbage_postgame_advances ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE private.cribbage_postgame_advances FROM PUBLIC, anon, authenticated;

CREATE OR REPLACE FUNCTION public.cribbage_advance_postgame(
  _game_id uuid,
  _round_id uuid,
...
BEGIN
  ...
  IF v_active_count < 2 OR v_active_human_count < 1 THEN
    RAISE EXCEPTION 'cribbage_advance_postgame:insufficient_active_cohort';
  END IF;
...
REVOKE ALL ON FUNCTION public.cribbage_advance_postgame(uuid, uuid, uuid, integer)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.cribbage_advance_postgame(uuid, uuid, uuid, integer)
