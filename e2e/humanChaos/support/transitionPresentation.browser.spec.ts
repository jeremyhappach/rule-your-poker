...
test(`actual Holm Chucky flip renderer: shortened CSS=${shortened}`, async ({ browser }) => {
  const bundle = await build({
    stdin: { contents: `import React,{useEffect,useState} from 'react'; import {createRoot} from 'react-dom/client';
      import {HolmChuckyRevealCard} from './src/components/HolmChuckyRevealCard';
      function App(){const [revealed,setRevealed]=useState(false);useEffect(()=>{setTimeout(()=>setRevealed(true),100)},[]);
        return <div style={{display:'flex'}}>{[0,1,2,3].map(i=><div key={i} data-holm-card-id={'r#chucky-'+i} style={{width:60,height:90}}>
          <HolmChuckyRevealCard card={{rank:'K',suit:'♥'}} presentationKey={'r#chucky-'+i} revealed={revealed}/></div>)}</div>}
      createRoot(document.getElementById('root')).render(<App/>);`,
      resolveDir: process.cwd(), loader: 'tsx' },
    bundle: true, write: false, format: 'iife', jsx: 'automatic',
    // Card artwork is immaterial to flip timing. Keep the actual state machine,
    // DOM structure and transition; replace only its leaf artwork dependencies.
    plugins: [{ name: 'card-artwork', setup(build) {
      build.onResolve({ filter: /^\.\/(PlayingCard|canonicalShell\/CanonicalCardBack)$/ }, args => ({ path: args.path, namespace: 'artwork' }));
      build.onLoad({ filter: /.*/, namespace: 'artwork' }, args => ({ loader: 'tsx', resolveDir: process.cwd(), contents:
        `import React from 'react'; export const ${args.path.includes('PlayingCard') ? 'PlayingCard' : 'CanonicalCardBack'}=()=> <div style={{width:60,height:90,background:'gold'}}/>;` }));
    } }],
  });
  ...
  try {
    ...
    const observer = new TransitionPresentationObserver();
    await observer.attach(context, page);
    await page.route('https://holm-flip-control.invalid/**', route => route.fulfill({ contentType: 'text/html', body: `
      <style>${shortened ? '[data-holm-chucky-flip-card]>div{transition-duration:100ms!important}' : ''}</style>
      <div data-holm-presentation-scope='{"gameId":"g","dealerGameId":"d","roundId":"r","handNumber":1}'>
        ${[0, 1, 2, 3].map(i => `<div data-holm-card-id="r#community-${i}" data-holm-card-presentation="face" data-holm-card-flipping="0" style="width:30px;height:30px">card</div>`).join('')}
        <div id="root"></div></div><script>${bundle.outputFiles[0].text}</script>` }));
    await page.goto('https://holm-flip-control.invalid/');
    await expect.poll(() => observer.samples.some(row => row.holmCards?.chucky.length === 4
      && row.holmCards.chucky.every(card => card.state === 'revealed'))).toBe(true);
    const check = () => assertHolmReveals(observer.samples, { gameId: 'g', dealerGameId: 'd', roundId: 'r', handNumber: 1 }, 0, Date.now());
    if (shortened) expect(check).toThrow('shortened');
    else expect(check().flips).toHaveLength(4);
  } finally { await context.close(); }
});
}
...
for (const game of ['cribbage', 'yahtzee', 'gin', 'holm']) {
for (const { cancel, shortened } of [{ cancel: false, shortened: false }, { cancel: true, shortened: false }, { cancel: false, shortened: true }]) {
  test(`${game} payout CSS end precedes retirement, cancellation=${cancel}, shortened=${shortened}`, async ({ browser }) => {
    ...
    try {
      ...
      await observer.attach(context, page);
      await page.goto('data:text/html,<div id="root"></div>');
      const origin: ChipEndpointRef = game === 'holm' ? { kind: 'pot' } : { kind: 'seat', position: 1 };
      await page.evaluate(({ cancelEarly, originKind, shortened, game }) => {
        ...
        root.innerHTML = `<style>@keyframes __chipTransport_control {from{transform:translateX(0)}to{transform:translateX(100px)}}${shortened ? '[data-chip-transport-intent]{animation-duration:100ms!important}' : ''}</style>
          ...
          <div data-chip-transport-intent="payout" data-chip-transport-from="${originKind}" data-chip-transport-variant="canonicalWinTransfer" data-chip-transport-completes-at="${Date.now() + 550}" style="width:30px;height:30px;background:gold;animation:__chipTransport_control 500ms linear"></div>`;
      ...
      const payoutKind = game === 'holm' ? 'pot' : 'payout';
      await expect.poll(() => observer.samples.some(row => row.stages.some(stage => stage.kind === payoutKind))).toBe(true);
      await expect(page.locator('[data-chip-transport-intent]')).toHaveCount(0);
      await page.evaluate(() => new Promise<void>(resolve => requestAnimationFrame(() => requestAnimationFrame(() => resolve()))));
      expect(observer.samples.some(row => row.stages.some(stage => stage.kind === payoutKind && stage.finished))).toBe(!cancel && !shortened);
