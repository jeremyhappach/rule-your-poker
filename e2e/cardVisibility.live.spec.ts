...
for (const gameType of ['holm-game', '3-5-7'] as const) test(`${gameType} live table observes actual card loss with debug off`, async ({ browser }, info) => {
  ...
  try {
    ...
    expect((evidence.responsiveness as any).addedP95, 'Actual HUD click-to-paint added p95 budget').toBeLessThanOrEqual(5);
    expect((await events()).length, 'No repeated incident or false tab/pause alarms').toBe(1);
  } finally {
