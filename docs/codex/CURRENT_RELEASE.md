...
Based on deployed/main `f09fcc9df6ef999817d6437ea790f7ee179e6e5c`, the normal action path now performs fresh Auth getUser, the unchanged server reducer against a server-loaded snapshot, and one `run21_server_commit_admitted` transaction. That service-only RPC checks current admin/allowlist/gate and canonical table membership, locks the same match/game rows, checks current identity and CAS, then invokes the unchanged journal/settlement commit. No client role can invoke it; its actor UUID comes exclusively from fresh Auth output, preserving the existing privileged server trust boundary. No authorization cache exists. Duplicate/rejected command results also require current admission and CAS, without writing a duplicate. Cold starts or CAS races may load a computation snapshot; no action calls the standalone admission RPC. Read/history/stream admission and autonomous bot/recovery authority remain unchanged.

Additive migration: `20260922215153_run21_admission_in_commit.sql`; no historical migration, rules, presentation, Farkle or bounded-journal changes. Focused validation: 60 tests, typecheck/build, rollback/apply SQL proof including revocation/nonadmin/removed membership/role grants/no-op/CAS/journal/settlement, and 48 actual local handler/browser Place/Pass actions. Every local action used exactly one admitted-commit RPC and zero standalone admissions. Local auth p50/p95/max 71/104/150 ms; commit 44/84/86 ms; handler 132/184/193 ms; tap-to-frame 158.2/208/224.6 ms. The deterministic four-phase fixture is local evidence only. Production qualification follows migration-before-handler deployment.

Reconciled candidate `c1dafd579350aaa149f64cf794196eea35b0760b` onto final Farkle main `4f9968d8386ab8e8e8298fd4209d5a24e583c34f` without source or shared-owner conflicts. Renumbered the migration to `20260922210908_run21_bounded_live_journal.sql` after the Farkle head and updated its proof path. The installed production CLI rejected the first LOCK with SQLSTATE 25P01 because it does not add a transaction; catalog evidence confirmed no objects or history entry were created. Added explicit BEGIN/COMMIT around the otherwise unchanged migration body, retaining rollback/apply proof transaction ownership. Run21 presentation, rules, and Farkle files remain unchanged.
...
September 21: the setup-timeout handoff is closed entirely inside the isolated
Farkle continuation owner. Terminal frames retain Farkle; setup/waiting retire
the live family into the existing neutral canonical state. No additional shared
owner, guard or grant changed. The complete 158-assertion rollback proof passes,
including actual authenticated-role negatives, canonical setup timeout, duplicate/
...
approved +20 ms added-p95 budget. Final added p95 is -13.7 ms / -7.6 ms; this
shows no measured regression, not a proven speedup. All application/harness
tests, eight mobile-engine fixture tests and four real-table tests pass.
Bounded observation records privacy-safe incidents without debug UI; changing
card admission now invalidates pending confirmation. No gameplay or schema
change is included. The original Hand 2 disappearance remains unresolved.
See [CARD_VISIBILITY_RELEASE_20260915.md](CARD_VISIBILITY_RELEASE_20260915.md)
for qualification, exact coverage, limitations and recovery.
...
- Jeremy renewed the no-play window. The first host-win live row passed one
  ordinary leg on both browsers, then stopped: the harness expected round 1
  while the authoritative game correctly advanced to round 2. Original traces
  and screenshots are retained; exact fake-session cleanup was independently
  verified. Jeremy approved the harness-only correction: exact hand/round
