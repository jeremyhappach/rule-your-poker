...
| Full-seam campaign ledger | `e2e/fullSeam/manifest.ts` is the single versioned inventory for branch, terminal, lifecycle, and rule-requirement coverage. Its focused test enforces stable unique identities, exact cross-references, explicit missing-driver dispositions, and the Cribbage fixture binding. It records planned coverage; only retained exact-build evidence can turn a row into a pass. |
