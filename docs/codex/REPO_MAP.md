...
| Admin fake-money smoke teardown | `admin_blast_fake_money_game` in `supabase/migrations/20260815180000_admin_blast_fake_money_game.sql`; additive `20260922233733_farkle_admin_blast_cleanup_claim.sql` enters an exact Farkle cleanup claim after admin/fake-money checks and restores it after cascade. Forward recovery and focused proof: `supabase/farkle/cleanup-claim/`. `PlayerOptionsMenu.tsx` exposes blast only through the admin/fake-money guard in `Game.tsx`, whose DELETE listener returns connected clients to the lobby. |
