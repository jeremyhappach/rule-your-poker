...
try{
 ...
 const migration=['20260922210908_run21_bounded_live_journal.sql','20260922215153_run21_admission_in_commit.sql'].map(file=>fs.readFileSync('supabase/migrations/'+file,'utf8').replace(/^BEGIN;\r?\n/m,'').replace(/^COMMIT;\s*$/m,'')).join('\n');
 ...
 BEGIN PERFORM public.run21_server_commit_admitted('${f.user}','${f.row.dealer_game_id}',0,NULL,NULL);RAISE EXCEPTION 'revoked gate allowed';EXCEPTION WHEN insufficient_privilege THEN NULL;END;
 INSERT INTO private.run21_release_allowlist(user_id) VALUES('${f.user}');
 DELETE FROM public.user_roles WHERE user_id='${f.user}' AND role='admin';
 BEGIN PERFORM public.run21_server_commit_admitted('${f.user}','${f.row.dealer_game_id}',0,NULL,NULL);RAISE EXCEPTION 'nonadmin allowed';EXCEPTION WHEN insufficient_privilege THEN NULL;END;
